...
flowchart TD
    ...
    R -->|"얼굴·몸으로 조작"| H["🎥 입력 추상화<br/>카메라 입력"]
    R -->|"고요하고 아름다운 것"| I["🏛️ 고요한 경험<br/>불가능 기하 · 아트"]
    R -->|"여럿이 함께 놀고 싶다"| J["👥 같은 자리와<br/>단말 간 동기화"]

    A & B & C & D & E & F & G & H & I & J --> Q["✅ 모든 답변이 통과하는<br/>5가지 질문"]
...
```
SKILL.md              라우터. 10개 경로, 5가지 질문, 넘지 말아야 할 선
references/core/      어포던스, MDA, 인지 부하, 리듬과 공감각, 입력 추상화, 카메라 입력, 불가능 기하, 고요한 경험, 여럿이 함께 놀기
